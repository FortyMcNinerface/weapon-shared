...
export const App = () => {
  ...
  const { holder, health, deadHolder, attacker } = s
  ...
  useEffect(() => {
    ...
    const name = world.getAvatar(deadHolder).name
    world.emit('hfy-death', { uid: deadHolder }) //triggers graveyard app to respawn deadHolder
    world.chat(`${name} was killed by ${attacker}! Respawning...`) //${attacker}
    dispatch('resetDeath')
  }, [deadHolder])
  ...
  useEffect(() => {
    ...
    const onPointerUp = e => {
      ...
      if (hit?.entity.isAvatar && hit.distance < attackRange) {
        ...
        const attacker2 = world.getAvatar(holder).name
        world.chat(`${attacker2} hit for ${dmg} damage`) // proof that the attacker name is captured
        world.emit('katana-attack', {
          uid: hit.entity.uid,
          dmg,
        })
      }
      ...
      {holder ? (
          ...
          <global>
            ...
            <group ref={weaponRef}>
              <model
                ...
                scale={scale}
              />
              <audio ref={attackRef} src={attackSfx} spatial volume={0.3} />
            </group>
          </global>
        </>
      ) : (
        <>
          <model
            ...
            scale={scale}
          />
          <audio ref={equipRef} src={equipSfx} spatial volume={0.5} />
        </>
      )}
      ...
      {holder && holder === localUser && (
        <>
          ...
          <group position={[1, 0, 10]}>
            <model src="orb_blue.glb" />
            <trigger
              size={[2, 2, 2]}
              onEnter={() =>
                world.emit('katana-attack', {
                  uid: world.getAvatar()?.uid,
                  dmg: 50,
                })
              }
              debug={false}
    ...
    actions: {
      ...
      damage(state, holder, dmg) {
        ...
        if (state.health <= 0) {
          ...
          // world.chat(`${holder} was killed by ${attacker2}! Test 2`) //crashes
        }
      },
